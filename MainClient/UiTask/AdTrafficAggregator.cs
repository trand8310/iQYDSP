...
namespace MainClient.UiTask
{
    ...
    public sealed class AdTrafficAggregator : IAsyncDisposable, IDisposable
    {
        ...
        public void EnqueueFetchedIp(int taskId, int count = 1)
        {
            ...
            _proxyIpStateQueue.Writer.TryWrite(
                new AdTrafficTaskProxyIpStateEvent(
                    taskId,
                    AdTrafficProxyIpKind.Fetched,
                    null,
                    count));
        }
        ...
        /// 获取指定任务的执行状态
        /// </summary>
        public AdTrafficTaskStateEntity? GetTaskStateById(int taskId)
        {
            return _taskStates.TryGetValue(taskId, out var stats) ? stats : null;
        }

        /// <summary>
        /// 获取主机的执行状态
        /// </summary>
        public AdTrafficTaskStateEntity GetHostTaskStats()
        {
            return _hostTaskStates;
        }
        ...
        public async Task FlushOnceAsync(CancellationToken token = default)
        {
            ...
            foreach (var pair in _proxyIpStates)
            {
                ...
                if (snapshot.Fetched > 0)
                    metrics["fetched"] = snapshot.Fetched;

                if (snapshot.Consumed > 0)
                    metrics["consumed"] = snapshot.Consumed;
        ...
        private async Task WaitConsumersDrainAsync()
        {
            ...
            if (_processTaskQueue != null)
                tasks.Add(_processTaskQueue);

            if (_processIpQueue != null)
                tasks.Add(_processIpQueue);
        ...
        #region Baseline Init - Newtonsoft.Json
        ...
        private async Task EnsureTaskBaselineAsync(int taskId, double taskCtr)
        {
            ...
            try
            {
                ...
                var json = ToJObject(resp);

                if (json != null)
                {
                    globalStats.Start = json.SelectToken("data.start")?.Value<long>() ?? 0;
                    globalStats.DSP = json.SelectToken("data.dsp")?.Value<long>() ?? 0;
                    globalStats.Clickthrough = json.SelectToken("data.click")?.Value<long>() ?? 0;
                }
        ...
        /// <summary>
        /// 把 AdeHelper.GetTaskStatusAsync 返回值统一转成 Newtonsoft.Json.Linq.JObject。
        /// 兼容 JObject、JToken、string、普通 object。
        /// </summary>
        private static JObject? ToJObject(object? resp)
        {
            if (resp == null)
                return null;

            if (resp is JObject jObject)
                return jObject;

            if (resp is JToken jToken)
                return jToken as JObject;

            if (resp is string jsonText)
            {
